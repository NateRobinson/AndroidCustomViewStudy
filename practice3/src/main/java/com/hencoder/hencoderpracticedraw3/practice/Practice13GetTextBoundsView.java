...
public class Practice13GetTextBoundsView extends View {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        offY = -(rect.top+rect.bottom)/2;
        canvas.drawText(text1, 100, middle+offY, paint2);

        paint2.getTextBounds(text2,0,text2.length(),rect);
        offY = -(rect.top+rect.bottom)/2;
        canvas.drawText(text2, 200, middle+offY, paint2);

        paint2.getTextBounds(text3,0,text3.length(),rect);
        offY = -(rect.top+rect.bottom)/2;
        canvas.drawText(text3, 300, middle+offY, paint2);

        paint2.getTextBounds(text4,0,text4.length(),rect);
        offY = -(rect.top+rect.bottom)/2;
        canvas.drawText(text4, 400, middle+offY, paint2);

        paint2.getTextBounds(text5,0,text5.length(),rect);
        offY = -(rect.top+rect.bottom)/2;
        canvas.drawText(text5, 500, middle+offY, paint2);

        paint2.getTextBounds(text6,0,text6.length(),rect);
        offY = -(rect.top+rect.bottom)/2;
        canvas.drawText(text6, 600, middle+offY, paint2);
    }
}